clear  % 原文 cluster-size control：删除 <15 体素的连通簇；R 对应 R/clusters3d.R::keep_clusters_by_extent()。
%%%%%%%%% Cluster control %%%%%%%%
...
load('C:\Users\KeBo\Documents\GitHub\KeBo2023_EmotionReg_BayesFactor\Final_SystemComponentMap_Consensus_BeforeClusterControl.mat')  %%Load BF maps；R 版直接读取 *_before_cluster_*.nii.gz。

%%Save them in a easier name ^^ %%%%
BF_tstat1=CommonAppraisal;  % Common appraisal 二值图。
BF_tstat2=ReappraisalOnly;  % Reappraisal only 二值图。
BF_tstat3=NonModifiableEmo;  % Non-modifiable emotion 二值图。
BF_tstat4=ModifiableEmo;  % Modifiable emotion 二值图。

Subregion1=region(BF_tstat1);  % MATLAB region() 拆分连通簇；R 版用 mmand::components 或纯 R BFS。
...
for i=1:size(Subregion1,2)  % Search across all clusters and only select the one that is larger than 15 voxels
    ...
    if (length(Subregion1(i).val))>14  % “>14” 等价于至少 15 体素；R 版参数名为 min_voxels=15。
...
BF_tstat1_CC=region2fmri_data(Subregion1_CC(1:end),BF_tstat1)  % R 版 keep_clusters_by_extent() 直接返回过滤后的逻辑数组，不再转回 fmri_data。
