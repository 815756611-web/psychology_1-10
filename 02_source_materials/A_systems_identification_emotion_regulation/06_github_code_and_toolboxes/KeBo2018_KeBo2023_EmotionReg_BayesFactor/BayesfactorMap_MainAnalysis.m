clear  % 原文主分析脚本：生成 t 图、2*log(BF) 图和四系统二值图；R 对应 route_A_original_reproduction/scripts/02_original_reproduction.R + R/stats_maps.R + R/bayes_factor.R。
...
load('D:\CANlab_Working\CANlab_Note\Emotion Regulation\drive-download-20210518T210113Z-001\PIP_WithoutRest3_Whole_Beta.mat')  % MATLAB 直接读绝对路径 .mat；R 版改成相对目录中的 NIfTI 索引扫描。



RegMinusNeg = image_math(Whole_Reg, Whole_Neg,'minus' )  %%% Compute Regulate negative vs Look negative；R 对应 reappraisal_effect = reg - neg。
NegMinusNeu = image_math(Whole_Neg, Whole_Neu,'minus' )  %%% Compute Look negative vs Look neutral；R 对应 emotion_generation = neg - neu。

%%% Generate T maps for Regulate negative vs Look negative 
tRegNeg=ttest(RegMinusNeg);  % MATLAB 直接对 image object 做单样本 t；R 版用 compute_contrast_stats(...,"reappraisal_effect") 显式累计均值/方差。
...
tNegNeu= ttest(NegMinusNeu);  % R 版对应 compute_contrast_stats(...,"emotion_generation")。
...
BF_RegNeg=estimateBayesFactor(tRegNeg,'t');   %%%% 输出是 2*log(BF)；R 对应 t_to_two_log_bf(..., method="jzs_exact")。
BF_NegNeu=estimateBayesFactor(tNegNeu,'t');  % R 版 full 默认也使用 JZS exact，并保留 bic_approx 作为快速近似。
