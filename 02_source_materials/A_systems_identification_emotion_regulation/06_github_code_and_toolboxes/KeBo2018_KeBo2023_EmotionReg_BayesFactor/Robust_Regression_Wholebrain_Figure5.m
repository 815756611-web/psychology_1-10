clear  % 原文 Figure 5 的 whole-brain robust regression；R 当前没有逐体素 robust 回归等价实现，这是与主复现项目的一个明确差异点。


%%%%%%%%%%%%%%%%%
% load('D:\CANlab_Working\BF_Mat\AHAB_PIP_Combined.mat')
load('C:\Users\KeBo\Dropbox (Dartmouth College)\2021_Ke_Bo_reappraisal_Gianaros\Data\fMRI_data\AHAB_FullData_Meta.mat')  % 直接读取 .mat；R 版先把行为和 NIfTI 镜像到相对路径。

RegNeg_AHAB=image_math(Whole_Reg,Whole_Neg,'minus');  % R 对应 reappraisal_effect。
NegNeu_AHAB=image_math(Whole_Neg,Whole_Neu,'minus');  % R 对应 emotion_generation。
...
success=[Success_AHAB;Success_PIP];  % R 对应 behavior_clean.csv 里的 reg_success。
...
image_obj = apply_mask(image_obj, maskdat);         % Mask - analyze gray-matter；R Route A 主复现没有单独加 gray matter mask 这一步。
...
image_obj = rescale(image_obj, 'rankvoxels');     % rankvoxels 是原文 Figure 5 的稳健性处理；R 当前未复做 whole-brain 行为回归，所以这里只做对应说明。
...
out = regress(image_obj, 'robust');  % 核心 robust whole-brain regression；R 当前没有直接对应，仅保留 cluster/ROI 层行为相关摘要。
